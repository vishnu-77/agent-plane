...
| --- | --- | --- | --- |
| POST | `/v1/authorize` | agent | Decide `{task, action, resource}`; optional `approval` (resume) and `context` (provenance). 200 ALLOW top-level; 202 / 403 wrap the decision in `detail`. |
| POST | `/v1/leases` | admin | Issue a lease from a full document (flat or manifest shape). |
| POST | `/v1/leases/from-template` | admin | Issue from `{template, subject, task, variables, id?}`. |
