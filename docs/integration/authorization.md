...
2. Choose **Connect access** and enter that server's `ADMIN_TOKEN` as the
   operator token. An agent bearer token can be added alongside it to use the
   request workbench and read tenant usage.
3. Run the example above and refresh.

| Page | What it shows |
| --- | --- |
| Overview | Gateway readiness, storage, identity mode, recent decisions. |
| Authority leases | Operator inventory across tenants with status and use counts; issue, narrow, delegate, or revoke. |
| Approvals | Pending approval requests; approve or reject with a note, which lets the executor resume that exact action once. |
| Decisions | The signed audit chain, searchable and filterable, with a per-event inspector. |
| Policies, Usage | Active policy bundle and reload; metered usage for the connected tenant. |
| Request workbench | Send model, tool, or retrieval requests through the gateway as the connected agent. |

Tokens are held only in the tab's memory and sent only to the page's own
origin; Disconnect clears them. Authorization evidence is not proof of
execution, and current lease data is not a historical snapshot.
...
| `/v1/approvals` | Operators list, approve, and reject pending requests. | The console's Approvals page calls these with the operator token. |
...
the TypeScript SDK, the Helm chart, and the committed OpenAPI export. Existing lint and dependency-audit jobs
