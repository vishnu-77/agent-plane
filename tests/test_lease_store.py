...
def test_for_subject_task_and_list(db_url):
    ...
    store.add(_lease(id="l4", tenant="other-tenant"))
    assert {lease.id for lease in store.for_subject_task("agent", "task")} == {"l1"}
    assert {lease.id for lease in store.for_subject_task("agent", "task", "other-tenant")} == {"l4"}
    assert {lease.id for lease in store.list()} == {"l1", "l2", "l3", "l4"}
