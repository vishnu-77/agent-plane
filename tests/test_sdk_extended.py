...
def test_admin_issues_and_manages_leases(admin, plane):
    lease = admin.issue_lease(id="lease-sdk", subject="sdk-agent", task="t1",
...
def test_admin_templates_and_audit(admin, plane):
    ...
    lease = admin.issue_from_template("repair-service", subject="sdk-agent", task="t2",
...
def test_approval_round_trip_via_sdk(admin, plane):
    admin.issue_lease(id="lease-appr", subject="sdk-agent", task="t3", resources=["staging/*"],
...
def test_delegate_via_sdk(admin, plane, server):
    admin.issue_lease(id="lease-parent", subject="sdk-agent", task="t4", resources=["staging/*"],
...
def test_context_round_trips(admin, plane):
    admin.issue_lease(id="lease-ctx", subject="sdk-agent", task="t5", resources=["staging/*"],
...
def test_govern_decorator_runs_only_on_allow(admin, plane):
    admin.issue_lease(id="lease-dec", subject="sdk-agent", task="t6", resources=["staging/*"],
...
def test_govern_raises_approval_required(admin, plane):
    admin.issue_lease(id="lease-dec2", subject="sdk-agent", task="t7", resources=["staging/*"],
                      actions=["deployment.restart"], require_approval=["deployment.restart"])
...
def test_governed_dispatch_and_openai_guard(admin, plane):
    admin.issue_lease(id="lease-disp", subject="sdk-agent", task="t8",
...
def test_langchain_style_tool_wrapper(admin, plane):
    admin.issue_lease(id="lease-lc", subject="sdk-agent", task="t9", resources=["staging/*"],
