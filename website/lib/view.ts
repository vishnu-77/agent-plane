...
export const VIEW_EVENT = "agentplane:view";
export const VIEW_STORAGE_KEY = "agentplane-view";
export const BUILDER_ANCHORS = ["connect", "rules", "integrations"];
export const RESEARCH_ANCHORS = ["model", "decision", "honest"];
...
export const viewScript = `(function(){try{var d=document.documentElement,v="builder",q=new URLSearchParams(location.search).get("view"),h=location.hash.slice(1);if(q==="research"||q==="builder"){v=q}else if(${JSON.stringify(
  RESEARCH_ANCHORS,
)}.indexOf(h)>-1){v="research"}else if(${JSON.stringify(
  BUILDER_ANCHORS,
)}.indexOf(h)>-1){v="builder"}else if(localStorage.getItem("${VIEW_STORAGE_KEY}")==="research"){v="research"}d.dataset.view=v}catch(e){}})();`;
