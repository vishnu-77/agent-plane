...
export class AgentPlaneAdmin extends BaseClient {
  ...
  async issueLease(input: IssueLeaseInput): Promise<Lease> {
    const { status, text } = await this.call("POST", "/v1/leases", {
      protected_resources: [], require_approval: [], max_uses: {}, maximum_impact: "reversible",
      child_authority: "none", ...input,
    });
    return asLease(parseJson(status, text).lease);
  }

  async issueFromTemplate(template: string, input: { subject: string; task: string; variables?: Record<string, string>; id?: string }): Promise<Lease> {
    const { status, text } = await this.call("POST", "/v1/leases/from-template", { template, variables: {}, ...input });
    return asLease(parseJson(status, text).lease);
  }
